...
if(isset($_GET['id'])){
	include ('form_edit_team.php');
} else {
	include ('form_new_team.php');
}
...
if(isset($_GET['id'])){
	...
	if(isset($_POST['submit'])){
		...
		if(is_numeric($_POST['id'])){
			...
			if($name == '' || $description == '' || $link == ''){
				...
				if($stmt = $mysqli->prepare("UPDATE teams SET name = ?, description = ?, link = ?, created = NOW() WHERE id = ?")){
	...
	if(isset($_POST['send'])){
		...
		if($name == '' || $description == '' || $link == ''){
			...
			if($stmt = $mysqli->prepare('INSERT teams (name, description, link, created) VALUES (?,?,?, NOW())')){
